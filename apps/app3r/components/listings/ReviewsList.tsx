/**
 * W-Round-1 Wave 2: ReviewsList
 *
 * Renders D86 reviews (visible only — Backend filters server-side).
 * Owner replies displayed inline under each review.
 * Server Component.
 */
import type { ReviewDto } from "@/lib/types/listing-meta";
    ...
    <section className="bg-white border border-gray-200 rounded-xl p-6">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <h2 className="text-lg font-bold text-gray-900">
          รีวิว (Reviews) <span className="text-gray-500 text-sm font-normal">({reviews.length})</span>
        </h2>
      ...
      <ul className="space-y-4">
        {reviews.map((r) => (
          <li key={r.id} className="border-l-4 border-website-brand-200 pl-4 py-2">
              ...
              <ul className="mt-3 ml-4 pl-4 border-l-2 border-gray-200 space-y-2">
                {r.replies.map((rep) => (
                  <li key={rep.id} className="text-xs text-gray-700">
                    <span className="font-semibold text-website-brand-700">
