/**
 * W-Round-1 Wave 2: QuestionsList
 *
 * Renders GR-5 questions for anonymous public visitors.
 * Backend filter (anonymous = isVisible=true only) applied server-side.
 * Server Component.
 */
import type { QuestionDto } from "@/lib/types/listing-meta";
...
export function QuestionsList({ questions, isClosed }: Props) {
  return (
          ...
          {questions.map((q) => (
            <li key={q.id} className="border-l-4 border-website-brand-200 pl-4 py-2">
              <div className="flex items-start justify-between gap-2 mb-2 flex-wrap">
                ...
                <span className="text-xs text-gray-500 shrink-0">{formatDateTh(q.createdAt)}</span>
              </div>
                ...
                <ul className="mt-2 ml-4 pl-4 border-l-2 border-gray-200 space-y-2">
                  {q.replies.map((rep) => (
                    <li key={rep.id} className="text-xs text-gray-700">
                      <span className="font-semibold text-website-brand-700">ตอบ:</span>{" "}
