'use client'
// Sub-5a D80 list + Sub-5b drawer/CRUD — content
...
export default function ContentPage() {
  const { filters, pagination, setFilters, setPage, resetMockData, filteredItems, createItem, updateItem, removeItem } = useAdminContentStore()
  ...
  const openView = (row: ContentRecord) => { setSelected(row); setMode('view'); setOpen(true) }

  const handleSubmit = async (data: unknown) => {
    if (mode === 'create') createItem(data as Omit<ContentRecord, 'id' | 'createdAt'>)
    else if (selected) setSelected(updateItem(selected.id, data as Partial<ContentRecord>))
  }
