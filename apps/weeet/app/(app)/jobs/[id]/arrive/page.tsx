...
export default function ArrivePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  return (
    <div className="pb-6">
      ...
      <div className="px-4 pt-4 space-y-4">
        <div className="bg-blue-950/40 border border-blue-800/60 rounded-xl p-3 text-xs text-blue-300 space-y-1">
          <p className="font-semibold">📸 ต้องถ่ายอย่างน้อย 2 รูป:</p>
          <p>1. รูปจุดสังเกต / หน้าบ้านลูกค้า</p>
          <p>2. รูปร่วมกับลูกค้า (แสดงตัวตน)</p>
        </div>
        ...
        <button
          ...
          disabled={!canSubmit}
          className="w-full bg-blue-600 hover:bg-blue-500 disabled:bg-gray-600 disabled:cursor-not-allowed text-white font-semibold py-3.5 rounded-xl transition-colors flex items-center justify-center gap-2"
        >
          {submitting ? (
