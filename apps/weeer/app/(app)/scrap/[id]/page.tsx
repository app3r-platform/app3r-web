...
// ── WeeeR Scrap Detail + Offer — R-71 (S1-S4, S8, S12) ──────────────────────
...
const AnnoOriginR71 = () => (
  <div className="mock-anno mock-anno-origin text-[10px] bg-yellow-50 border border-yellow-200 rounded-lg px-3 py-1 text-yellow-700 font-mono">
    ◀ มาจาก: R-70 · /scrap (feed) หรือ R-72 · /scrap/browse (browse + filter)
  </div>
);
const AnnoXAppR71 = ({ id }: { id: string }) => (
  <details className="mock-anno mock-anno-xapp">
    <summary className="cursor-pointer text-xs bg-purple-50 border border-purple-200 text-purple-700 rounded-lg px-3 py-1.5 inline-flex items-center gap-1.5 font-medium">
      👁 แอพฯอื่น ณ จังหวะนี้ (R-71: รายละเอียดซาก + ยื่นข้อเสนอ)
    </summary>
    <div className="mt-1 bg-purple-50 border border-purple-200 rounded-xl p-3 text-xs text-purple-800 space-y-1">
      <p>• <strong>WeeeU :3002</strong> [U-33] เจ้าของซากเห็น listing เดียวกัน (seller view)
        <a href={`http://localhost:3002/scrap/${id}`} className="underline ml-1">/scrap/[id]</a>
      </p>
      <p>• หลัง "ยืนยัน + Lock Escrow" → WeeeU [U-33] เห็น pending_offer; ถ้า WeeeU ยืนยัน → R-28</p>
      <p>• <strong>Admin :3000</strong> [A-08] Admin เห็นงานใหม่หลัง offer ถูกยอมรับ
              ...
              <div className="flex gap-2">
                <div className="flex-1">
                  <button type="submit" disabled={submitting}
                    className="w-full bg-[#FF663A] hover:bg-[#D8491F] text-white font-semibold py-2.5 rounded-xl text-sm transition-colors disabled:opacity-60">
                    {submitting ? "กำลังส่ง…" : "✅ ยืนยัน + Lock Escrow"}
                  </button>
                  {/* §6 Nav annotation */}
                  <p className="mock-anno mock-anno-nav text-[10px] text-blue-500 font-mono mt-1">→ R-71 (แสดง Escrow locked) → รอ WeeeU ยืนยัน → R-28 /scrap/jobs/[jobId]</p>
                </div>
