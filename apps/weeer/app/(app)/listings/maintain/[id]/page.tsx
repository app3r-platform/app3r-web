// ── listings/maintain/[id]/page.tsx — WeeeR Maintain Job Detail (Sub-1 D4) ───
...
export default async function MaintainDetailPage({ params }: Props) {
  ...
  if (!session) {
    ...
    redirect("/login?reason=weeer-required&from=/listings/maintain/" + id);
  }

  // ── Load job ─────────────────────────────────────────────────────────────────
  const job = getMaintainJobById(id);
  if (!job) notFound();
  ...
  return (
    <div className="space-y-5 max-w-2xl">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
        <h2 className="text-sm font-semibold text-gray-800 mb-2">📝 รายละเอียดงาน</h2>
